...
def buscar_expressoes(paginas):  
    logger.info("Iniciando busca de expressões em %s página(s)", len(paginas))  
    resultados = []
        ...
        for expressao in EXPRESSOES_ALVO:
            ...
            for variacao in expressao["variacoes"]:
                ...
                for match in re.finditer(
                    ...
                    logger.info("Ocorrência localizada: %s (página %s, busca %s)", nome, numero_pagina, "exata")
            # ==============================
            # 2. BUSCA FLEXÍVEL
            # ==============================
            ...
            for match in re.finditer(
                ...
                logger.info("Ocorrência localizada: %s (página %s, busca %s)", nome, numero_pagina, "flexivel")
    return resultados
...
def processar_pdf(pdf_file):
    ...
    paginas = extrair_texto_estruturado(pdf_file)
    resultados = buscar_expressoes(paginas)
    return resultados
